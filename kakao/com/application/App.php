...
class App extends Application{
	...
	function facebokConnect(){
			...
			if(isset($this->session->getSession('facebook_session','facebook')->user)) {
				$this->assign('fbConnect',"index.php?".$this->Request->encrypt_params(array("page"=>"vote","act"=>"dovote","uid"=>@$this->session->getSession('facebook_session','facebook')->user)));
		...
		if($CONFIG['MAINTENANCE']==true){
			...
			// $this->gplusConnect();
				...
				if($this->Request->getParam("act")=='detail'){
					...
					$this->assign('globalURL', $CONFIG['BASE_DOMAIN']);
	...
	function run(){
		...
		if( $page != '' ){
			...
			$this->assign('fburl',$content->getLogoutUrl());

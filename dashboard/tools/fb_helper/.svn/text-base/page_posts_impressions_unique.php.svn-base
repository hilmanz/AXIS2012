<?php
	set_time_limit(0);
require "config.php";
  $my_url = "http://nonatetramedia.com/fb/page_posts_impressions_unique.php"; //=> set your redirect url
  ...
  // success
	...
	$metrix = "page_posts_impressions_unique";
	$proID = 9;
	// SETTING PARAMETER
	...
	foreach($data2 as $dt2){
		$day = date("Y-m-d",strtotime($dt2->end_time));
		$q = "INSERT IGNORE INTO fb_".$metrix."_week (proID,value,day) 
				VALUES ('".$proID."','".$dt2->value."','".$day."')";
		if(mysql_query($q)){
		echo $q." => SUKSES<br>";
		}else{
		echo $q." => GAGAL<br>";
	...
	// INPUT DATA PER 28DAYS
	foreach($data3 as $dt3){
		$day = date("Y-m-d",strtotime($dt3->end_time));
		$q = "INSERT IGNORE INTO fb_".$metrix."_days_28 (proID,value,day) 
				VALUES ('".$proID."','".$dt3->value."','".$day."')";
		if(mysql_query($q)){
		echo $q." => SUKSES<br>";
		}else{
		echo $q." => GAGAL<br>";

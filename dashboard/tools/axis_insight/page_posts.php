...
$log->info("Initial range since ".date("d-m-Y H:i:s",SINCE)." until ".date("d-m-Y H:i:s",UNTIL));
$redirect_url = CANVAS_URL.$metrix.".php"; //=> set your redirect url
$paging = "";     
//$since = "";
$until = "";
$request_post_url = $_GET["post"];
if(!empty($request_post_url)){
    //Store it in session
    $_SESSION["post"] = $request_post_url; 
  ...
  if(empty($access_token)){
      $log->info("warning: \"code\" parameter is empty or not valid, application will continue running");
  }      
...
//  $log->info("info: response from the graph: {$response}");
  ...
  // check to see if this is an oAuth error:
    ...
    $request_post_url = $_SESSION["post"];
    if(!empty($request_post_url)){
	$urlPost = $request_post_url;
	$log->info("Getting post url from request");
    }
    else{
	$urlPost = "https://graph.facebook.com/".$pagesID."/posts?access_token=".$access_token."&since=".SINCE."&until=".UNTIL;
	$log->info("Getting initial post url");
    }
    
    $log->info("{$metrix} URL POST: {$urlPost}");
    $params = null;
    parse_str($urlPost, $params);
    $until = date("d-m-Y H:i:s",$params["until"]);
    $log->info("processing post from {$until}");
    $content = file_get_contents($urlPost);
    $var = json_decode($content);
    $posts = $var->data;
    $paging = $var->paging;
    
    foreach($posts as $post){
	    $page_like_count = $post->id;
	    $from_name = $post->from->name;
	    $from_category = $post->from->category;
	    $from_id = $post->from->id;
	    $message = mysql_escape_string($post->message);
	    $comment_count = $post->comments->count;
	    $like_count = $post->likes->count;
	    if(empty($like_count)){$like_count = 0;}
	    $created_time = convert_facebook_timestamp($post->created_time);
	...
	if(!$STOP){    
	    $query = "INSERT IGNORE INTO fb_page_posts (post_id,from_name,from_category,from_id,message,comment_count,like_count,created_time) 
	...
	$query = "SELECT DATE_FORMAT(created_time,'%Y-%m-%d') as post_date, COUNT(id) as post_count FROM fb_page_posts GROUP BY DATE_FORMAT(created_time,'%Y-%m-%d')";
	$result = mysql_query($query);
	while($row=  mysql_fetch_array($result)){
	    $insert = "INSERT INTO fb_page_posts_summary (post_date,post_count) VALUES('{$row["post_date"]}',{$row["post_count"]}) ON DUPLICATE KEY UPDATE post_count={$row["post_count"]}";
...
<html>
	...
	<script type="text/javascript">
	    ...
	    var next_url = "<?=$_SERVER["PHP_SELF"]."?post=".urlencode($paging->next)?>";

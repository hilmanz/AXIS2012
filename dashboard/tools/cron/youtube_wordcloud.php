<?php
/**
 * generating the top keyword for youtube video comments data
...
include "db.php";
class youtube_wordcloud extends db{
	protected $last_id;
	...
	public function init(){
		...
		$this->debug->setAppName('youtube_wordcloud');
	...
	private function retrieve_feeds($last_id){
		$sql = "SELECT 
				...
				comment as content	 
				FROM 
				axis_report_2012.channel_video_comment
	...
	private function add_keyword($words){
		if(is_array($words)){
			foreach($words as $keyword){
				...
				if(strlen($keyword)>2){
					$sql = "INSERT INTO `axis_report_2012`.`tbl_youtube_wordcloud` 
							( 
							`keyword`, 
							`total`
							)

...
include "db.php";
class gplus_wordcloud extends db{
	protected $last_id;
	...
	public function init(){
		...
		$this->debug->setAppName('gplus_wordcloud');
	...
	private function retrieve_feeds($last_id){
		$sql = "SELECT 
				...
				content	 
				FROM 
				axis_report_2012.tbl_gplus_feed
	...
	private function add_keyword($words){
		if(is_array($words)){
			foreach($words as $keyword){
				...
				if(strlen($keyword)>2){
					$sql = "INSERT INTO `axis_report_2012`.`tbl_gplus_wordcloud` 
							( 
							`keyword`, 
							`total`
							)

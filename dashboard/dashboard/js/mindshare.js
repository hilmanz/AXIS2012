		...
		chart = new Highcharts.Chart({
			...
			xAxis: {
				...
				labels: {
					...
					formatter: function(){
						var dateFormat = (this.value).substr(8,2)+'/'+(this.value).substr(5,2);
						return dateFormat;
					}
				}
			},
			...
			tooltip: {
				formatter: function() {
					var dateFormat = (this.x).substr(8,2)+'/'+(this.x).substr(5,2)+'/'+(this.x).substr(0,4);
					return '<b>'+ this.series.name +'</b><br/>'+
					dateFormat +': '+ Highcharts.numberFormat(this.y, 0);
				}
			},
			plotOptions: {
				area: {
...
function lineChart(divID, category, data, catID, _legend){
	var chart;	
	if(_legend == true){
		legendz = false;
	}else{

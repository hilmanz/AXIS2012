...
class Admin extends SQLData{
	...
	function showDashboard(){
		...
		for($i=0;$i<sizeof($projectList);$i++){
			...
			}else if($tabWeb[$i] == 'Active'){
				$update[$i] = $dashboard->getLastUpdateWeb($projectList[$i]["id"]);
			}else if($tabSocial[$i] == 'Active'){
				$update[$i] = $dashboard->getLastUpdateSocial($projectList[$i]["id"]);
			}
			// var_dump($update[$i]);exit;
		}
		
		//hard code
		$_SESSION['lastUpdateAXIS'] = $update[0]['lastUpdate'];
		...
		$this->View->assign("tabWeb",$tabWeb);
...
//  	$this->View->assign('pro2', $projectList[1]['name']);
	...
	function showLoginPage(){
		if($_GET['f']==1){
			$this->View->assign("msg","Username/password is incorrect !");
		}else if($_GET['f']==2){
			$this->View->assign("msg","Wrong security code! Please try again.");
		}

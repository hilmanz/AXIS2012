...
class DashboardManager extends SQLData{
	...
	function getLastUpdateWeb($id){
		$this->open(0);
		$q=	"SELECT DATE_FORMAT(last_updated_date,'%d %M %Y') AS lastUpdate
			FROM rpt_axis_dashboard
	...
	function getProjectList($id){
		$this->open(0);
// 		var_dump($page);exit;
// 		$project = "SELECT tbl_project.id, name, DATE_FORMAT(start_date,'%d %M %Y') AS mulai, seo, sem, social,web, kpi, project_status, channel_stats.channel_name as cname, tbl_project.channel_id, description FROM tbl_project JOIN channel_stats WHERE tbl_project.userID='".$id."' GROUP BY tbl_project.id LIMIT 5";
		$project = "SELECT c.id, name, DATE_FORMAT(c.start_date,'%d %M %Y') AS mulai, c.seo, c.sem, c.social, c.web, c.project_status, c.channel_id AS cname, c.description

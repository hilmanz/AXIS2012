...
class sharePostHelper {
	...
	function uploadThisImage($files=null){
		...
		list($file_nameThumb,$ext_thumb) = explode('.',$files['name']);
		$img_thumb = md5($files['name'].rand(1000,9999)).".".$ext_thumb;
		try{
		...
		if(move_uploaded_file($files['tmp_name'],$path.$filename.'.'.$type[1])){
				...
				list($width, $height, $filetype, $attr) = getimagesize("{$path}{$filename}.{$type[1]}");
					$maxSize = 720;
					if($width>=$maxSize){
						if($width>=$height) {
							$subs = $width - $maxSize;
					...
					$big = $thumb->save( "{$path}big_{$filename}.{$type[1]}");
					$thumb->adaptiveResize($w_small,$h_small);
					$small = $thumb->save( "{$path}medium_{$filename}.{$type[1]}" );
					$thumb->adaptiveResize($w_tiny,$h_tiny);
					$tiny = $thumb->save( "{$path}thumb_{$filename}.{$type[1]}");
	...
	function getSocialStatArticle($qData=null){
	...
	function getAuthorProfile($aid=null){
		...
		$sql = "SELECT id, name, 	fb_id,gplus_id,twitter_id,img, '' as emblem FROM social_member WHERE id IN ({$aid}) LIMIT 50 ";
	...
	function getDetailContent(){
		...
		FROM social_news_content snc
		LEFT JOIN axis_news_content_category ancc ON ancc.id = snc.categoryid
		WHERE snc.id={$id} AND n_status IN ({$this->server}) LIMIT 1 ";

...
class socialMediaHelper {
	...
	function addSocialMediaDataNoneAxis(){
		...
		if(in_array(true,$arrSomed)){
			...
			if(@$this->apps->session->getSession('gplus_session','gplus')->user) {
				$socmedID[] = "gplus_id = '{$gplus_id}'";
				$registerby = 'gplus';
			}
			if(@$this->apps->session->getSession('twitter_session','twitter')->user){
				$socmedID[] = "twitter_id = '{$twitter_id}'";
				$registerby = 'twitter';
			}
			if(@$this->apps->session->getSession('facebook_session','facebook')->user) {
				$socmedID[] = "fb_id = '{$fbid}'";
				$registerby = 'facebook';
			}
			
			if($socmedID){
				$qSocMedID  = " ON DUPLICATE KEY UPDATE ";
				$qSocMedID .= implode(',',$socmedID);
			}
			$sql = " 
			INSERT INTO social_member 
			(fb_id,twitter_id,gplus_id,email,name,sex,phone_number,password,register_date,salt,n_status,register_by) 
			VALUES ('{$fbid}','{$twitter_id}','{$gplus_id}',\"{$email}\",\"{$name}\",'{$sex}','{$phonenumber}','{$hashPass}',NOW(),'{$CONFIG['salt']}',1,'{$registerby}')
			{$qSocMedID}

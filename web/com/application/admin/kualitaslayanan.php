...
class kualitaslayanan extends Admin{
	...
	function home(){
		...
		$totalList = $this->fetch("SELECT count(*) total
			FROM axis_coorporate_blog con
			WHERE n_status<>3
			AND con.type={$this->type} GROUP BY con.parentid",1);
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=kualitaslayanan&lid={$this->lid}"));	
		return $this->View->toString("application/admin/kualitaslayanan/kualitaslayanan_list.html");
	}
	...
	global $CONFIG;
		...
		if($this->_p('save')){
			if( $lid=='' || $title==''|| $content==''){
				...
				return $this->View->toString("application/admin/kualitaslayanan/kualitaslayanan_new.html");
			}
			...
			if(!$last_id){
				...
				if ($_FILES['image']['name']!=NULL) {
					...
					if(move_uploaded_file($_FILES['image']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}kualitaslayanan/{$img}")){
						list($width, $height, $type, $attr) = getimagesize("{$CONFIG['LOCAL_PUBLIC_ASSET']}kualitaslayanan/{$img}");
						$maxSize = 1000;
						if($width>=$maxSize){
							if($width>=$height) {
								$subs = $width - $maxSize;
						...
						$big = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}kualitaslayanan/".$img);
						$thumb->adaptiveResize($w_small,$h_small);
						$small = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}kualitaslayanan/small_".$img );
						$thumb->adaptiveResize($w_tiny,$h_tiny);
						$tiny = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}kualitaslayanan/tiny_".$img );
					}
				...
				return $this->View->showMessage('Berhasil', "index.php?s=kualitaslayanan");
			}
		}
		return $this->View->toString("application/admin/kualitaslayanan/kualitaslayanan_new.html");
	}
	...
	function add_language(){
		...
		if(!$this->getLastInsertId()){
			return $this->View->showMessage('Gagal', "index.php?s=kualitaslayanan");
		}else{
			return $this->View->showMessage('Berhasil', "index.php?s=kualitaslayanan");
		}
	}
	...
	function edit(){
		...
		if(! $this->_p('save')){
			...
			if( $lid=='' || $title=='' || $content==''){
				$this->View->assign('msg',"Please complete the form!");
				return $this->View->toString("application/admin/kualitaslayanan/kualitaslayanan_edit.html");
			}

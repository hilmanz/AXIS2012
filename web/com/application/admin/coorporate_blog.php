...
class coorporate_blog extends Admin{
	...
	function home(){
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=coorporate_blog&lid={$this->lid}&startdate={$startdate}&enddate={$enddate}"));	
		return $this->View->toString("application/admin/blog/blog_list.html");
	}
	...
	function add(){
		...
		if($this->_p('save')){
			if( $lid=='' || $title=='' ||  $content==''){
				...
				return $this->View->toString("application/admin/blog/blog_new.html");
			}
			...
			if(!$last_id){
				...
				if ($_FILES['image']['name']!=NULL) {
					...
					if(move_uploaded_file($_FILES['image']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}blog/{$img}")){
						list($width, $height, $type, $attr) = getimagesize("{$CONFIG['LOCAL_PUBLIC_ASSET']}blog/{$img}");
						$maxSize = 1000;
						if($width>=$maxSize){
							if($width>=$height) {
								$subs = $width - $maxSize;
						...
						$big = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}blog/big_".$img);
						$thumb->adaptiveResize($w_small,$h_small);
						$small = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}blog/small_".$img );
						$thumb->adaptiveResize($w_tiny,$h_tiny);
						$tiny = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}blog/tiny_".$img );
					}
	...
	function edit(){
		...
		if(! $this->_p('save')){
			...
			if( $lid=='' || $title=='' || $content==''){
				$this->View->assign('msg',"Please complete the form!");
				return $this->View->toString("application/admin/blog/blog_edit.html");
			}

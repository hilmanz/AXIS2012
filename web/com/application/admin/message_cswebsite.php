...
class message_cswebsite extends Admin{
	...
	function home(){
		...
		$sql = "
			...
			AND msg.date_time > '2012-12-12 00:00:01'
		...
		$sql = "SELECT count(*) total FROM axis_message WHERE n_status<>3 AND date_time > '2012-12-12 00:00:01' AND tipe IN ({$strCid}) {$filter} ";
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=message_cswebsite&search={$this->_g('search')}&startdate={$this->_g('startdate')}&enddate={$this->_g('enddate')}"));	
		return $this->View->toString("application/admin/cswebsite/cs_website.html");
	}
	...
	function detail(){
		...
		if( is_array($list_message) ){
			...
			return $this->View->toString("application/admin/cswebsite/detail_csWebsite.html");
		}else{
			return $this->View->showMessage('Invalid id', "index.php?s=message_cswebsite");
		}
		return $this->View->toString("application/admin/cswebsite/detail_csWebsite.html");
	}
	...
	function hapus(){
		...
		if( !$this->query("UPDATE axis_message SET n_status=3 WHERE id={$id}")){
			return $this->View->showMessage('Gagal', "index.php?s=message_cswebsite");
		}else{
			return $this->View->showMessage('Berhasil', "index.php?s=message_cswebsite");
		}
	}
	...
	function export_excel(){
		...
		$excel->getExcel($r,'message_csWebsite');
		exit;
	}
}
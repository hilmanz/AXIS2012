...
class mediasiaran extends Admin{
	...
	function home(){
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=mediasiaran&lid={$this->lid}"));	
		return $this->View->toString("application/admin/mediasiaran/mediasiaran_list.html");
	}
	...
	global $CONFIG;
		...
		if($this->_p('save')){
			if( $lid=='' || $title==''|| $content==''){
				...
				return $this->View->toString("application/admin/mediasiaran/mediasiaran_new.html");
			}
			...
			if(!$last_id){
				...
				if ($_FILES['image']['name']!=NULL) {
					...
					if(move_uploaded_file($_FILES['image']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}mediasiaran/{$img}")){
						list($width, $height, $type, $attr) = getimagesize("{$CONFIG['LOCAL_PUBLIC_ASSET']}mediasiaran/{$img}");
						$maxSize = 1000;
						if($width>=$maxSize){
							if($width>=$height) {
								$subs = $width - $maxSize;
						...
						$big = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}mediasiaran/".$img);
						$thumb->adaptiveResize($w_small,$h_small);
						$small = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}mediasiaran/small_".$img );
						$thumb->adaptiveResize($w_tiny,$h_tiny);
						$tiny = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}mediasiaran/tiny_".$img );
					}
				...
				return $this->View->showMessage('Berhasil', "index.php?s=mediasiaran");
			}
		}
		return $this->View->toString("application/admin/mediasiaran/mediasiaran_new.html");
	}
	...
	function add_language(){
		...
		if(!$this->getLastInsertId()){
			return $this->View->showMessage('Gagal', "index.php?s=mediasiaran");
		}else{
			return $this->View->showMessage('Berhasil', "index.php?s=mediasiaran");
		}
	}
	...
	function edit(){
		...
		if(! $this->_p('save')){
			...
			if( $lid=='' || $title=='' || $content==''){
				$this->View->assign('msg',"Please complete the form!");
				return $this->View->toString("application/admin/mediasiaran/mediasiaran_edit.html");
			}

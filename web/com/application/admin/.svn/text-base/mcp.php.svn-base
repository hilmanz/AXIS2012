...
class mcp extends Admin{
	...
	function home(){
		...
		if($list){
			...
			foreach($list as $key => $val){
				...
				//$list[$key]['deviceid'] = $deviceArr;
				$list[$key]['deviceid'] = implode(',',$arrDevice);
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=mcp&lid={$this->lid}&id_cat={$id_cat}&startdate={$startdate}&enddate={$enddate}"));	
		return $this->View->toString("application/admin/mcp/mcp_list.html");
	}
	...
	function add(){	
		...
		//$content 	  = $this->fixTinyEditor( $this->Request->getPost('content') );
		...
		$url 			= $this->Request->getPost('url');
			...
			if( $lid=='' || $title=='' || $brief=='' || $content==''){
				...
				return $this->View->toString("application/admin/mcp/mcp_new.html");
			}
			$sql = "INSERT INTO axis_news_content 
				(url,lid,title,brief,prize,content,articleType,categoryid,n_status,created_date,posted_date,expired_date,authorid) 
				VALUES 
				('{$url}','{$lid}','{$title}','{$brief}','{$prize}','{$content}','{$this->type}','{$categoryid}','{$status}',NOW(),'{$posted_date}','{$expired_date}',{$this->Session->getVariable("uid")})";
			$this->query($sql);
			...
			if(!$last_id){
				...
				//insert to device
				$this->inputdevice($last_id,$id_device);
				...
				if ($_FILES['image']['name']!=NULL) {
					...
					if(move_uploaded_file($_FILES['image']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}mcp/{$img}")){
						list($width, $height, $type, $attr) = getimagesize("{$CONFIG['LOCAL_PUBLIC_ASSET']}mcp/{$img}");
						$maxSize = 1000;
						if($width>=$maxSize){
							if($width>=$height) {
								$subs = $width - $maxSize;
						...
						$big = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}mcp/big_".$img);
						$thumb->adaptiveResize($w_small,$h_small);
						$small = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}mcp/small_".$img );
						$thumb->adaptiveResize($w_tiny,$h_tiny);
						$tiny = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}mcp/tiny_".$img );
					}
				...
				return $this->View->showMessage('Berhasil', "index.php?s=mcp");
			}
		}
		return $this->View->toString("application/admin/mcp/mcp_new.html");
	}
	...
	function add_language(){
		...
		(url,parentid,lid,title,brief,prize,content,image,articleType,categoryid,n_status,created_date,posted_date,expired_date,authorid) 
		VALUES 
		('{$qData['url']}',{$qData['parentid']},{$lid},'{$qData['title']}','{$qData['brief']}','{$qData['prize']}','{$qData['content']}','{$qData['image']}',{$qData['articleType']},{$qData['categoryid']},'{$qData['n_status']}',NOW(),'{$qData['posted_date']}','{$qData['expired_date']}',{$this->Session->getVariable("uid")})";
		// pr($sql);exit;
		$this->query($sql);
		if(!$this->getLastInsertId()){
			return $this->View->showMessage('Gagal', "index.php?s=mcp");
		}else{
			return $this->View->showMessage('Berhasil', "index.php?s=mcp");
		}
	}
	...
	function edit(){
		...
		if(! $this->_p('save')){
			...
			if( $lid=='' || $title=='' || $brief=='' || $content==''){
				$this->View->assign('msg',"Please complete the form!");
				return $this->View->toString("application/admin/mcp/mcp_edit.html");
			}
	...
	function inputImage($last_id=null,$img=null){
		...
		$this->query("UPDATE axis_news_content SET image='{$img}' WHERE  parentid={$last_id} AND lid={$this->lid} LIMIT 1");
		return false;
	}
	...
	function getCategoryList(){
		$type = $this->fetch("SELECT * FROM axis_news_content_category WHERE used=1",1);
		return $type;
	}

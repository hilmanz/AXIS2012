...
class coverage extends Admin{
	...
	function home(){
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=coverage&search={$search}&lid={$this->lid}"));	
		return $this->View->toString("application/admin/coverage/coverage_list.html");
	}
	...
	function add(){
		...
		if($this->_p('save')){
			...
			if( $lid==''){
				...
				return $this->View->toString("application/admin/coverage/coverage_new.html");
			}
			$sql = "INSERT INTO axis_news_content 
				(lid,title,brief,content,articleType,cityid,n_status,created_date,authorid) 
				VALUES 
				('{$lid}',\"{$title}\",\"{$brief}\",\"{$content}\",'{$this->type}','{$cityid}','{$status}',NOW(),{$this->Session->getVariable("uid")})";
			$this->query($sql);
			...
			if(!$last_id){
				...
				if ($_FILES['image']['name']!=NULL) {
					...
					if(move_uploaded_file($_FILES['image']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}coverage/{$img}")){
						list($width, $height, $type, $attr) = getimagesize("{$CONFIG['LOCAL_PUBLIC_ASSET']}coverage/{$img}");
						$maxSize = 1000;
						if($width>=$maxSize){
							if($width>=$height) {
								$subs = $width - $maxSize;
						...
						$big = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}coverage/big_".$img);
						$thumb->adaptiveResize($w_small,$h_small);
						$small = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}coverage/small_".$img );
						$thumb->adaptiveResize($w_tiny,$h_tiny);
						$tiny = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}coverage/tiny_".$img );
					}
				...
				return $this->View->showMessage('Berhasil', "index.php?s=coverage");
			}
		}
		return $this->View->toString("application/admin/coverage/coverage_new.html");
	}
	...
	function add_language(){
		...
		if(!$this->getLastInsertId()){
			return $this->View->showMessage('Gagal', "index.php?s=coverage");
		}else{
			return $this->View->showMessage('Berhasil', "index.php?s=coverage");
		}
	}
	...
	function edit(){
		...
		if(! $this->_p('save')){
			...
			if( $lid==''){
				$this->View->assign('msg',"Please complete the form!");
				return $this->View->toString("application/admin/coverage/coverage_edit.html");
			}
	...
	function getProvince(){
		$province = $this->fetch("SELECT * FROM axis_province_reference WHERE  id NOT IN (3,15,23,7,33,34,31,35,29,36)   ORDER BY province ASC ",1);
		return $province;
	}

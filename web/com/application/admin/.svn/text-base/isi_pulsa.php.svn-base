...
class isi_pulsa extends Admin{
	...
	function home(){
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=isi_pulsa&search={$search}&lid={$this->lid}"));	
		return $this->View->toString("application/admin/isi_pulsa/isi_pulsa_list.html");
	}
	...
	function add(){
		...
		if($this->_p('save')){
			...
			if($lid==''){
				...
				return $this->View->toString("application/admin/isi_pulsa/isi_pulsa_new.html");
			}
			...
			if(!$last_id){
				...
				if ($_FILES['image']['name']!=NULL) {
					...
					if(move_uploaded_file($_FILES['image']['tmp_name'],"{$CONFIG['LOCAL_PUBLIC_ASSET']}isi_pulsa/{$img}")){
						list($width, $height, $type, $attr) = getimagesize("{$CONFIG['LOCAL_PUBLIC_ASSET']}isi_pulsa/{$img}");
						$maxSize = 1000;
						if($width>=$maxSize){
							if($width>=$height) {
								$subs = $width - $maxSize;
						...
						$big = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}isi_pulsa/".$img);
						$thumb->adaptiveResize($w_small,$h_small);
						$small = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}isi_pulsa/small_".$img );
						$thumb->adaptiveResize($w_tiny,$h_tiny);
						$tiny = $thumb->save( "{$CONFIG['LOCAL_PUBLIC_ASSET']}isi_pulsa/tiny_".$img );
					}
					$this->inputImage($last_id,$img);
				}
				return $this->View->showMessage('Berhasil', "index.php?s=isi_pulsa");
			}
		}
		return $this->View->toString("application/admin/isi_pulsa/isi_pulsa_new.html");
	}
	...
	function add_language(){
		...
		if(!$this->getLastInsertId()){
			return $this->View->showMessage('Gagal', "index.php?s=isi_pulsa");
		}else{
			return $this->View->showMessage('Berhasil', "index.php?s=isi_pulsa");
		}
	}
	...
	function edit(){
		...
		if(! $this->_p('save')){
			...
			if($lid==''){
				$this->View->assign('msg',"Please complete the form!");
				return $this->View->toString("application/admin/isi_pulsa/isi_pulsa_edit.html");
			}

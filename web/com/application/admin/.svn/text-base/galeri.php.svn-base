...
class galeri extends Admin{
	...
	function home(){
		...
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=galeri&lid={$this->lid}"));	
	...
	function savecrop(){
		...
		print json_encode(array('image'=>$CONFIG['BASE_DOMAIN_PATH'].'public_assets/galeri/thumb_'.$files['source_file']));
		exit;
	}

...
class master extends Admin{
	...
	function list_type(){
		...
		$total_per_page = 20;
		$sql = "SELECT * FROM axis_news_content_type ORDER BY id LIMIT {$start},{$total_per_page}";
		$totalSql = "SELECT count(*) total FROM axis_news_content_type";
		$this->open(0);
	...
	function add_type_news(){
		...
		if($this->Request->getParam('cmd') == 'add'){
			if( $type != ''){
				$qry = "INSERT INTO axis_news_content_type (type) VALUES ('{$type}')";
				if(!$this->query($qry)){
	...
	function edit_type_news(){
		...
		if( $id > 0 && $type != ''){
			$qry = "UPDATE axis_news_content_type SET type='{$type}' WHERE id={$id}";
			if($this->query($qry)){
				return $this->View->showMessage('Berhasil', "index.php?s=master&act=list_type");
			}else{
				$this->View->assign('msg','Error, please fill all field!');
		...
		$list_type = $this->fetch("SELECT * FROM axis_news_content_type WHERE id={$id}");
	...
	function delete_type_news(){
		$id = $this->Request->getParam('id');
		if( !$this->query("DELETE FROM axis_news_content_type WHERE id={$id}")){
	...
	function list_category(){
		$start = intval($this->Request->getParam('st'));
		$total_per_page = 20;
		$sql = "SELECT * FROM axis_news_content_category ORDER BY id LIMIT {$start},{$total_per_page}";
		$totalSql = "SELECT count(*) total FROM axis_news_content_category";
		$this->open(0);
		$list = $this->fetch($sql,1);		
	...
	function add_category_news(){
		...
		if($this->Request->getParam('cmd') == 'add'){
			if( $category != ''){
				$qry = "INSERT INTO axis_news_content_category (category,naming,point) VALUES ('{$category}','{$naming}','{$point}')";
				if(!$this->query($qry)){
					$this->View->assign("msg","Add process failure");
	...
	function edit_category_news(){
		...
		if( $id > 0 && $category != '' && $naming != ''){
			$qry = "UPDATE axis_news_content_category SET category='{$category}',naming='{$naming}',point='{$point}' WHERE id={$id}";
			if($this->query($qry)){
				return $this->View->showMessage('Berhasil', "index.php?s=master&act=list_category");
			}else{
				$this->View->assign('msg','Error, please fill all field!');
		...
		$list_category = $this->fetch("SELECT * FROM axis_news_content_category WHERE id={$id}");
	...
	function delete_category_news(){
		$id = $this->Request->getParam('id');
		if( !$this->query("DELETE FROM axis_news_content_category WHERE id={$id}")){
	...
	function list_page(){
		$start = intval($this->Request->getParam('st'));
		$total_per_page = 20;
		$sql = "SELECT * FROM axis_news_content_page ORDER BY id LIMIT {$start},{$total_per_page}";
		$totalSql = "SELECT count(*) total FROM axis_news_content_page";
		$this->open(0);
		$list = $this->fetch($sql,1);
		$tot_category = $this->fetch($totalSql);
		$this->close();
	...
	function add_page_news(){
		...
		if($this->Request->getParam('cmd') == 'add'){
			if( $pageName != ''){
				$qry = "INSERT INTO axis_news_content_page (pagename,pagetitle,n_status) VALUES ('{$pageName}','{$pagetitle}','{$n_status}')";
				if(!$this->query($qry)){
					$this->View->assign("msg","Add process failure");
	...
	function edit_page_news(){
		...
		if( $id > 0 && $pagename != ''){
			$qry = "UPDATE axis_news_content_page SET pagename='{$pagename}',pagetitle='{$pagetitle}',n_status='{$status}' WHERE id={$id}";
			if($this->query($qry)){
				return $this->View->showMessage('Berhasil', "index.php?s=master&act=list_page");
			}else{
				$this->View->assign('msg','Error, please fill all field!');
		...
		$list_page = $this->fetch("SELECT * FROM axis_news_content_page WHERE id={$id}");
	...
	function hapus_page_news(){
		$id = $this->Request->getParam('id');
		if( !$this->query("DELETE FROM axis_news_content_page WHERE id={$id}")){
	...
	function list_device(){
		$start = intval($this->Request->getParam('st'));
		$total_per_page = 20;
		$sql = "SELECT * FROM axis_news_device_type ORDER BY id LIMIT {$start},{$total_per_page}";
		$totalSql = "SELECT count(*) total FROM axis_news_device_type";
		$this->open(0);
		$list = $this->fetch($sql,1);
		$tot_category = $this->fetch($totalSql);
		$this->close();
	...
	function add_device(){
		...
		if($this->Request->getParam('cmd') == 'add'){
			if( $type != ''){
				$qry = "INSERT INTO axis_news_device_type (type) VALUES ('{$type}')";
				if(!$this->query($qry)){
					$this->View->assign("msg","Add process failure");
	...
	function edit_device(){
		...
		if($type != ''){
			$qry = "UPDATE axis_news_device_type SET id='{$id_device}',type='{$type}' WHERE id={$id}";
			if($this->query($qry)){
				return $this->View->showMessage('Berhasil', "index.php?s=master&act=list_device");
			}else{
				$this->View->assign('msg','Error, please fill all field!');
		...
		$list_device = $this->fetch("SELECT * FROM axis_news_device_type WHERE id={$id}");
	...
	function delete_device(){
		$id = $this->Request->getParam('id');
		if( !$this->query("DELETE FROM axis_news_device_type WHERE id={$id}")){
	...
	function list_province(){
		...
		$filter = $search!='' ? " AND province LIKE '%{$search}%' OR province_en LIKE '%{$search}%' " : ""; 
		$start = intval($this->Request->getParam('st'));
		$total_per_page = 20;
		$sql = "SELECT * FROM axis_province_reference WHERE s<>0 AND w<>0 AND e<>0 AND n<>0  {$filter} ORDER BY province LIMIT {$start},{$total_per_page}";
		$totalSql = "SELECT count(*) total FROM axis_province_reference {$filter}";
		$this->open(0);
		$list = $this->fetch($sql,1);
	...
	function add_province(){
		...
		if($this->Request->getPost('cmd') == 'add'){
			if( $province != ''){
				$qry = "INSERT INTO axis_province_reference (province,province_en,n,s,e,w,cover) VALUES ('{$province}','{$province_en}','{$n}','{$s}','{$e}','{$w}','{$cover}')";
				if(!$this->query($qry)){
					$this->View->assign("msg","Add process failure");
	...
	function edit_province(){
		...
		if( $id > 0 && $province != ''){
			$qry = "UPDATE axis_province_reference SET province_en='{$province_en}', province='{$province}',n='{$n}',s='{$s}',e='{$e}',w='{$w}',cover='{$cover}' WHERE id={$id}";
			
			if($this->query($qry)){
				$img = md5($province.rand(1000,9999));
				$image = $this->fetch("SELECT image FROM axis_province_reference WHERE id={$id}");
				if ($_FILES['image_3g']['name']!=NULL) {
					if ($image['image']=='') {
						$img;
					} else {
		...
		$list_province = $this->fetch("SELECT * FROM axis_province_reference WHERE id={$id}");
		if( is_array($list_province) ){
			...
			$this->View->assign("image",$list_province['image']);
	...
	function hapus_province(){
		$id = $this->Request->getParam('id');
		if( !$this->query("UPDATE axis_province_reference SET s=0,w=0,e=0,n=0 WHERE id={$id}")){
	...
	function list_city(){
		...
		$start = intval($this->Request->getParam('st'));
		$total_per_page = 20;
		$sql = "SELECT * FROM axis_city_reference {$filter} ORDER BY provinceName LIMIT {$start},{$total_per_page}";
		$totalSql = "SELECT count(*) total FROM axis_city_reference {$filter}";
		$this->open(0);
		$list = $this->fetch($sql,1);
	...
	function add_city(){
		...
		$provinceName = $this->fetch("SELECT province FROM axis_province_reference WHERE id={$provinceid}");
		
		if($this->Request->getParam('cmd') == 'add'){
			if( $city != ''){
				$qry = "INSERT INTO axis_city_reference (provinceName,provinceid,city,city_en) VALUES ('{$provinceName['province']}','{$provinceid}','{$city}','{$city_en}')";
				if(!$this->query($qry)){
					$this->View->assign("msg","Add process failure");
	...
	function edit_city(){
		...
		if( $id > 0 && $city != ''){
			$qry = "UPDATE axis_city_reference SET provinceName='{$provinceName['province']}',provinceid='{$provinceid}',city='{$city}',city_en='{$city_en}' WHERE id={$id}";
			if($this->query($qry)){
				return $this->View->showMessage('Berhasil', "index.php?s=master&act=list_city");
			}else{				
				$this->View->assign('msg','Error, please fill all field!');
		...
		$list_city = $this->fetch("SELECT * FROM axis_city_reference WHERE id={$id}");
	...
	function hapus_city(){
		$id = $this->Request->getParam('id');
		if( !$this->query("DELETE FROM axis_city_reference WHERE id={$id}")){
	...
	function getProvince(){
		$province = $this->fetch("SELECT * FROM axis_province_reference ORDER BY province",1);
		return $province;
	}
	
	function inputImage($id,$img,$cover){
		$this->query("UPDATE axis_province_reference SET image='{$img}' WHERE id={$id};");
	}
}
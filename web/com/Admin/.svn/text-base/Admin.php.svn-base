...
class AdminConfig extends Admin{
	...
	function applyPermissions(){
		...
		foreach($id_category as $key=>$val){
			list($cat,$module) = explode('_',$val);
			$sql = "INSERT gm_specified_role(level,aid,type,category,module) VALUES('".$level."','".$id."',2,'".$cat."','".$module."')";
			$this->query($sql);
			/* if ($this->query($sql)) {
				foreach($id_type as $k=>$v){
					list($type,$id_module) = explode('_',$v);
					$sql = "UPDATE gm_specified_role SET type='{$type}' WHERE module='{$id_module}'";
					$this->query($sql);
				}			
			} */
		}
	...
	function showPermissionPage($msg=""){
		...
		$qData_type = $this->fetch("SELECT * FROM axis_news_content_type",1);
		$arrROle = $this->fetch("SELECT * FROM gm_specified_role WHERE aid={$user['userID']}",1);
		foreach($arrROle as $k=>$val){
			$cat_Role[$val['module']][$val['category']] = $val['category'];
			$cat_type[$val['module']][$val['type']] = $val['type'];
		}
		
		//foreach category	
		foreach($qData as $val){
			$categoryArr[$val['used']][$val['id']] = $val['category'];
			$categoryArr['3'][$val['used']][$val['id']] .= $val['category'];
		}
		//foreach type	
		foreach($qData_type as $val){
		...
		for($i=0;$i<sizeof($list);$i++){
			...
			if($list[$i]['requestID']=='article') {
				...
				$list[$i]['category'] = $categoryArr[$list[$i]['id_content']]; 
			} elseif ($list[$i]['requestID']=='article_user') {
				$list[$i]['id_content'] = $arr['article_user'];
				$list[$i]['category'] = $categoryArr[$list[$i]['id_content']][0];				
			}
			$list[$i]['type'] = $typeArr;
		}
		//print_r($cat_type);
		...
		$this->View->assign("cat_type",$cat_type);

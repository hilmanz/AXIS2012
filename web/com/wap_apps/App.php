	...
	function main(){
		...
		if($CONFIG['MAINTENANCE']==true){
			...
			$this->mainLayout(TEMPLATE_DOMAIN_WAP . '/master.html');
				...
				if(array_key_exists(strip_tags($this->_g('act')),$SEO)) $metaTag['title'] = $SEO[strip_tags($this->_g('act'))];
				else  $metaTag['title'] = false;
		
				if($metaTag['title']) $this->assign('currentpage',$metaTag);
				...
				$nexturl = urlencode64(serialize(array('basedomain'=>$CONFIG['WAP_DOMAIN'],'nexturl' => $_SERVER['REQUEST_URI'])));
		...
		if( $page != '' ){
			if( !is_file( $APP_PATH . MODULES_DOMAIN_WAP . $page . '.php' ) ){
				if( is_file( '../templates/'. TEMPLATE_DOMAIN_WEB . '/'. $page . '.html' ) ){
					...
					sendRedirect($CONFIG['WAP_DOMAIN']);
					die();
				}
			}else{

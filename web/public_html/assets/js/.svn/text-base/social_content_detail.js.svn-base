...
$(".poolingStarSocial").click(function(){
		...
		$.post(basedomain+'index.php?page=content&act=saveRateSocial',{cid:cid,rate:rate}, function(data){
		if(data){
			for(var x=1;x<=rate;x++){
		...
		var rate = $(this).attr("node");
			...
			$('.postCommentSocial').click(function(){
				...
				$.post(basedomain+'index.php?page=content&act=sendCommentSocial',{cid:cid,comment:comment},function(data){
				if(data){
					var html ='';
						html+='			 <div class="row">';
			...
			$(".favoriteSocial").click(function(){
				...
				$.post(basedomain+"index.php?page="+userpage+"&act=addMyFavorite&content=1&id="+contentid,function(data){
					if(data) {
						countFav++;

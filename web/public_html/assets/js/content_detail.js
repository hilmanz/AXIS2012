...
$(".poolingStar").mouseover(function(){
	...
	var rate = $(this).attr("node");
			for(var x=1;x<=rate;x++){
				...
				$(".node"+x).children("img").attr("src",basedomain+"assets/images/icon/stars.png");
	...
	var rate = $(this).attr("node");
			for(var x=1;x<=rate;x++){
				...
				$(".node"+x).children("img").attr("src",basedomain+"assets/images/icon/star.png");
...
$(".poolingStar").click(function(){
		...
		$.post(basedomain+'index.php?page=content&act=saveRate',{cid:cid,rate:rate}, function(data){
			if(data){
				for(var x=1;x<=rate;x++){
		...
		var rate = $(this).attr("node");
			...
			$('.postComment').click(function(){
				...
				$.post(basedomain+'index.php?page=content&act=sendComment',{cid:cid,comment:comment},function(data){
				if(data){
			...
			$(".rateVal").each(function(n,i){
				var rate = $(this).attr('rate');
					for(var x=1;x<=rate;x++){
						...
						$(".nodes"+x).children("img").attr("src",basedomain+"assets/images/icon/stars.png");
			...
			$(".favorite").click(function(){
				...
				var favorite =  $(this);
				$.post(basedomain+"index.php?page="+userpage+"&act=addMyFavorite&id="+contentid,{url_fav : urlFav},function(data){
					if(data) {
						countFav++;
		...
		$(".locale").click(function(){
			...
			$.post(basedomain+'index.php?page=locale&country='+locale,function(data){
	...
	$(".linkit").click(function(){ 	window.location=$(this).attr('href'); location.reload();});
...
function rateUser(){
			for(var x=1;x<=myRate;x++){
							...
							$(".nodeuser"+x).children("img").attr("src",basedomain+"assets/images/icon/stars.png");
